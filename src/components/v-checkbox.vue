                ...
                <div class="form">
                    <el-form :model="formDataC" ref="formDataC" label-width="100px" class="demo-dynamic">
                        ...
                        <el-form-item prop="order" :rules="[{ required: true, message: '题目序号不能为空', trigger: 'blur' }]">
                            <el-input placeholder="请输入题目序号" v-model="formDataC.order" type="number" oninput="if(value.length>2)value=value.slice(0,2)" min="0">
                        ...
                        <el-form-item v-if="showVido" label="多媒体链接" prop="url">
                            ...
                            <el-select v-model="formDataC.Types" placeholder="选择类型" prop="Types" class="select-type">
                                <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value"></el-option>
                            </el-select>
                            <!-- <el-button @click.prevent="removeTitleUrl()">删除</el-button> -->
                        </el-form-item>
                        <el-upload v-if="showAddVido" class="upload-demo" action="https://jsonplaceholder.typicode.com/posts/" :on-change="handleChange" :file-list="fileList">
                            <el-button size="small" type="primary">点击上传</el-button>
                            <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</div>
                        </el-upload>
                        <!-- 选项内容 -->
                        <div v-for="(option, index) in formDataC.options" :key="option.key">
                            <el-form-item :label="'选项' +`${index+1}`" :prop="'options.' + index + '.content'" :rules="[{ required: true, message: '选项不能为空', trigger: 'blur' }]">
                            ...
                            <el-form-item :prop="'options.' + index + '.title'" :rules="[{ required: true, message: '选项编号不能为空', trigger: 'blur' }]">
