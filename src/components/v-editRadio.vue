<template>
    <transition name="slide"
        v-if="formData.answerType===0||formData.answerType===1||formData.answerType===2||formData.answerType===5||formData.answerType===6||formData.answerType===7">
                    ...
                    <div class="form">
                        <el-form :model="formData" ref="formData" label-width="100px" class="demo-dynamic">
                            ...
                            <el-form-item prop="order"
                                ...
                                <el-input placeholder="请输入题目序号" v-model="formData.order" type="number"
                            ...
                            <el-form-item prop="url" v-if="showVido" label="多媒体链接">
                                <el-input placeholder="请输入链接" v-model="formData.url" class="input-with-select">
                                </el-input>
                            ...
                            <div v-if="formData.answerType===0||formData.answerType===1">
                                <div class="list" v-for="(option, index) in formData.items" :key="option.key">
                                    <el-form-item :label="'选项' +`${index+1}`" :prop="'items.' + index + '.content'"
                                    ...
                                    <el-form-item :prop="'items.' + index + '.title'"
                                    ...
                                    <el-form-item :prop="'items.' + index + '.order'"
                                    ...
                                    <el-form-item :prop="'items.' + index + '.point'">
                                        <el-input placeholder="需要流程控制请输入分值" type="number"
                                            oninput="if(value.length>2)value=value.slice(0,2)" :min="0"
                                    ...
                                    <el-form-item v-show="showItemVido" class="btn-vido">
                                        <el-form-item label="选择类型" style="margin-bottom:20px ;">
                                            <el-radio v-model="option.type" :label="0" border size="medium">文本
                                            </el-radio>
                                            <el-radio v-model="option.type" :label="1" border size="medium">图片
                                            </el-radio>
                                            <el-radio v-model="option.type" :label="2" border size="medium">音频
                                            </el-radio>
                                            <el-radio v-model="option.type" :label="3" border size="medium">视频
                                            </el-radio>
                                        </el-form-item>
