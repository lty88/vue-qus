  ...
  <div class="qs-list" >
    <div class="table">
      ...
      >
        ...
        <el-table-column align="center" width="260px" prop="title" label="问卷名称"></el-table-column>
        <el-table-column align="center" width="200px" prop="unitName" label="发起单位"></el-table-column>
        <el-table-column align="center" width="180px" prop="startTime" label="开始时间"></el-table-column>
        <el-table-column align="center" width="180px" prop="endTime" label="截止时间"></el-table-column>
        <el-table-column align="center" width="100px" prop="type" label="问卷类型">
          <template slot-scope="scope">{{ scope.row.type === 1 ? '指定性问卷': '开放性问卷' }}</template>
        </el-table-column>
        <el-table-column align="center" width="100px" prop="condition " label="问卷属性">
          <template slot-scope="scope">{{ scope.row.condition ===0 ? '静态问卷': '动态问卷' }}</template>
        </el-table-column>
        <el-table-column align="center" width="60px" prop="status" label="状态">
          <template
            slot-scope="scope"
          >{{ scope.row.status === 0 ? '禁用' : scope.row.status == 1?'启用' : '测试' }}</template>
        </el-table-column>
        <el-table-column align="center" label="操作" width="320px">
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              type="danger"
              @click="iterator = delItem(scope.row);iterator.next();"
            >
            ...
            <el-dropdown
              size="mini"
              split-button
              v-if="scope.row.condition===1 || scope.row.type===1"
            >
              高级设置
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item
                  ...
                  @click.native="condition(scope.row)"
                >
