    ...
    <div class="table">
      ...
      <el-table :data="tableData" style="width: 100%" :row-class-name="tableRowClassName">
        ...
        <el-table-column align="center" prop="artsci" label="文理属性">
          <template
            slot-scope="scope"
          >{{ scope.row.artsci === 0 ? '不分科' : scope.row.artsci == 1?'文科':2?'理科': '' }}</template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      headersObj: { Authorization: window.sessionStorage.getItem("token") }, //携带的请求头
      code: 1, //问卷code
      qsTitle: "", //问卷题目
      tableData: [], //参与人员表
  ...
  methods: {
    ...
    deleteAllUser() {
      this.$confirm("此操作将删除全部参与人员, 是否继续?", {
        ...
        .then(() => {
          deleteAllUser({ qnCode: this.code }).then(res => {
            console.log(res);
            this.fetchUserList();
          });
          this.$message({
    ...
    //获取问卷名称
    fetchDataQs() {
      getList({ code: this.code }).then(res => {
        console.log(res);
    ...
    //获取问卷参与人员信息
    fetchUserList() {
      getUser({ qnCode: this.code }).then(res => {
        console.log(res);
